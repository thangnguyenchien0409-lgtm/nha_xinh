import { getAllInspireApi, getInspireById } from '@/apis/inspireService';
...
export const fetchGetInspireById = createAsyncThunk<any, string, { rejectValue: string }>(
    'inspire/fetchGetInspireById',
    async (id, { rejectWithValue }) => {
        try {
            const res = await getInspireById(id);
            return res.data;
        } catch (error: any) {
            toast.error(error.message || 'Có lỗi xảy ra');
            return rejectWithValue(error.response?.data || error.message);
...
const inSpireSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            // get by id
            .addCase(fetchGetInspireById.pending, (state) => {
                state.status = 'loading';
            })
            .addCase(fetchGetInspireById.fulfilled, (state, action) => {
                state.status = 'idle';
                state.inspireById = action.payload;
            })
            .addCase(fetchGetInspireById.rejected, (state) => {
                state.status = 'error';
            });
    }
});
